...
def test_render_label_includes_abbreviated_client_bottom_right():
    ...
    assert "^FO316,154^FB194,1,0,R,0" in zpl
...
def test_render_label_preserves_template_structure():
    ...
    assert "^FO68,62^GB444,2,2^FS" in zpl  # divider with gap under EXT
    ...
    assert "^LH0,0" in zpl
    assert "^FO68,4^FDLOG# " in zpl
    assert "^FO120,76^BCN,70,N,N,N" in zpl
